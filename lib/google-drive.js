// lib/google-drive.js — Sube los PDF de posts a Google Drive, en
// Contenido/Redes sociales dentro de la misma carpeta raíz que usa
// bendito-os (Configuración → Drive), reutilizando el mismo refresh_token
// guardado en google_drive_auth. No hace falta autorizar nada aparte aquí.
...
/** Sube un PDF a Drive, en <raíz de bendito-os>/Contenido/Redes sociales. Devuelve el link o null si Drive no está conectado. */
async function subirPdfARedesSociales(nombreArchivo, buffer) {
  ...
  const carpetaRedes = await buscarOCrearCarpeta(drive, 'Redes sociales', carpetaContenido);

  const { Readable } = await import('stream');
  const stream = Readable.from(buffer);

  const res = await drive.files.create({
    requestBody: { name: nombreArchivo, parents: [carpetaRedes] },
...
module.exports = { subirPdfARedesSociales };
