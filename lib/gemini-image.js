...
async function llamarGeminiImagen(parts, systemInstruction) {
  ...
  // a los pocos segundos. Reintentamos un par de veces con espera antes de
  // rendirnos, en vez de fallar a la primera.
  //
  // OJO: un 429 con "limit: 0" en el mensaje NO es un límite agotado
  // temporalmente (esos indican un límite >0 ya consumido) — es que Google
  // ha dejado la cuota gratuita de este modelo a cero (nos pasó con
  // gemini-2.5-flash-image → resuelve a "gemini-2.5-flash-preview-image").
  // Reintentar no sirve de nada ahí (seguirá fallando siempre), así que se
  // corta al momento con un mensaje claro en vez de perder tiempo.
  ...
  for (let intento = 0; intento < 3; intento++) {
    ...
    if (res.status === 429 && /limit:\s*0\b/.test(errText)) {
      throw new Error(
        'Gemini ha quitado la cuota gratuita del modelo de edición de imagen (' + MODEL + '). ' +
        'Hace falta activar facturación en Google AI Studio para seguir usando "Editar con IA" — mientras tanto, ' +
        'usa "📤 Elegir imagen(es) final(es)" para subir la imagen editada a mano.'
      );
    }
